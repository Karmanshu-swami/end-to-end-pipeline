...
//				sh 'sudo docker rm -f $(sudo docker ps -a -q)'
		...
		stage("Prod Env") {
			steps {
			 sshagent(['ubuntu']) {
//			    sh 'sudo docker rm -f $(sudo docker ps -a -q)'
			    sh 'ssh -o StrictHostKeyChecking=no ubuntu@35.154.193.129 sudo docker rm -f $(sudo docker ps -a -q)' 
	                    sh "ssh -o StrictHostKeyChecking=no ubuntu@35.154.193.129 sudo docker run  -d  -p  49153:8080  karmanshu/javatest-app:$BUILD_TAG"
				}
			}
		}
	}
